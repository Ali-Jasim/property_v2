...
def read_tenant(tenant_id: int = Path(...), db: Session = Depends(get_db)):
...
@app.get("/tenants/")
def read_tenants(skip: int = Query(0), limit: int = Query(100), db: Session = Depends(get_db)):
...
def delete_tenant(tenant_id: int = Path(...), db: Session = Depends(get_db)):
...
def read_contractor(contractor_id: int = Path(...), db: Session = Depends(get_db)):
...
@app.get("/contractors/")
def read_contractors(skip: int = Query(0), limit: int = Query(100), db: Session = Depends(get_db)):
    return Contractor.get_all(db, skip, limit)

@app.put("/contractors/{contractor_id}")
def update_contractor(
    contractor_id: int = Path(...), 
    name: str = Query(None), 
    phone_number: str = Query(None), 
    email: str = Query(None), 
    db: Session = Depends(get_db)
...
def delete_contractor(contractor_id: int = Path(...), db: Session = Depends(get_db)):
...
def read_landlord(landlord_id: int = Path(...), db: Session = Depends(get_db)):
...
@app.get("/landlords/")
def read_landlords(skip: int = Query(0), limit: int = Query(100), db: Session = Depends(get_db)):
    return Landlord.get_all(db, skip, limit)

@app.put("/landlords/{landlord_id}")
def update_landlord(
    landlord_id: int = Path(...), 
    name: str = Query(None), 
    phone_number: str = Query(None), 
    email: str = Query(None), 
    db: Session = Depends(get_db)
...
def delete_landlord(landlord_id: int = Path(...), db: Session = Depends(get_db)):
...
def read_property(property_id: int = Path(...), db: Session = Depends(get_db)):
...
@app.get("/properties/")
def read_properties(skip: int = Query(0), limit: int = Query(100), db: Session = Depends(get_db)):
...
def delete_property(property_id: int = Path(...), db: Session = Depends(get_db)):
